...
open class MapPutNew {

    @Benchmark
    fun baseline(state: SingleFilledMapState) {
        state.baseline[randomIndex(state)*2 + 1] = state.element
    }

    @Benchmark
    fun custom(blackhole: Blackhole, state: SingleFilledMapState) {
        blackhole.consume(state.custom.put(randomIndex(state)*2 + 1, state.element))
    }

    @Benchmark
    fun functional(blackhole: Blackhole, state: SingleFilledMapState) {
        blackhole.consume(state.functional.update(randomIndex(state)*2 + 1, { state.element }, state.element))
    }
