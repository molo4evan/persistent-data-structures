...
import ru.nsu.mmp.pds.deque.FingerTree.*
...
class Empty<E> : FingerTree<E> {
    ...
    override fun concatWithMiddle(middle: List<E>, other: FingerTree<E>): FingerTree<E> {
        return if (middle.isEmpty()) {
            ...
            concatWithMiddle(middle.drop(1), other).pushFirst(middle.first())
        }
    }
}
...
class Deep<E>(
    ...
    override fun popFirst(): Pair<E, FingerTree<E>> {
        ...
        return if (list.size == 1) {
            ...
            Pair(value, Deep(Affix.fromList(list.drop(1)), deeper, suffix))
        }
    }
    ...
    override fun popLast(): Pair<E, FingerTree<E>> {
        ...
        return if (list.size == 1) {
            ...
            Pair(value, Deep(prefix, deeper, Affix.fromList(list.dropLast(1))))
        }
    }
    ...
    override fun concatWithMiddle(middle: List<E>, other: FingerTree<E>): FingerTree<E> {
        return when (other) {
            is Empty -> {
                ...
                else concatWithMiddle(middle.dropLast(1), Empty())
